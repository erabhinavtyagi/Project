...
/* ----------------------------------------------------------------------------- *\
		...
		description : it's a constructor for LoRa structure that assign default values
									and pass created object (LoRa struct instance)

		arguments   : Nothing

		returns     : A LoRa object with these default values:
											----------------------------------------
...
/* ----------------------------------------------------------------------------- *\
		...
		description : read a register(s) by an address and a length,
									then store value(s) at output array.
		arguments   :
			LoRa* LoRa        --> LoRa object handler
			uint8_t* address  -->	pointer to the beginning of address array
			uint16_t r_length -->	determines number of address bytes that
														you want to send
...
/* ----------------------------------------------------------------------------- *\
		...
		description : initialize and set the right setting according to LoRa struct vars
